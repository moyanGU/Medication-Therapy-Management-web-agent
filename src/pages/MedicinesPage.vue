<template>
  <div class="min-h-screen bg-gray-50">
          ...
          <div class="p-6">
            <div class="flex items-start justify-between mb-4">
              ...
              <div class="ml-4">
                <img
                  :src="
                    getImageUrl(medicine.image_path) || '/icons/app-icon.svg'
                  "
            ...
            <div class="space-y-2 mb-4">
              <div class="flex justify-between text-sm">
                ...
                <span
                  ...
                  >{{ medicine.quantity
                  }}{{ getMedicineTypeUnit(medicine.medicine_type) }}</span
                >
              </div>
              <div class="flex justify-between text-sm">
                ...
                <span
                  :class="{
                    'font-medium text-gray-900': !isExpiringSoon(
                      medicine.expiry_date
                    ),
                    'font-medium text-red-600': isExpiringSoon(
                      medicine.expiry_date
                    ),
                  }"
              ...
              <div class="flex justify-between text-sm">
                ...
                <span class="font-medium text-gray-900">{{
                  medicine.storage_conditions || '未设置'
                }}</span>
              </div>
            </div>
    ...
    <MedicineForm
      ...
      :draft-query="medicineDraftQuery"
      @close="closeDialog"
...
const medicineDraftQuery = computed<Record<string, string>>(() => {
...
const loadMedicines = async () => {
  try {
    ...
    if (isRequestCancelledError(err)) {
      console.log('🟡 [MedicinesPage] 加载药品列表请求已取消')
      return
    }
...
const closeDialog = async () => {
  ...
  if (route.query.source === 'page-agent') {
    await router.replace({ path: '/medicines' })
  }
}
...
const openAssistiveDraftIfNeeded = () => {
  if (route.query.source !== 'page-agent' || route.query.open_dialog !== '1') {
    return
  }

  console.log('[MedicinesPage] 打开页面助手药品草稿', medicineDraftQuery.value)
  editingMedicine.value = null
  showEditDialog.value = false
...
const getImageUrl = (imagePath: string | null | undefined) => {
  ...
  if (!imagePath) {
    console.log(
      '🔵 [MedicinesPage] imagePath is null/undefined, returning null'
    )
    return null
  }
  const fullUrl = resolveMediaUrl(imagePath)
  console.log('🔵 [MedicinesPage] resolved image URL:', fullUrl)
  return fullUrl
}
