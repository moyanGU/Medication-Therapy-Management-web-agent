        ...
        <section class="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
          <div class="flex items-center justify-between gap-4">
            ...
            <button
              v-if="serviceCase.plan?.completed_at"
              type="button"
              class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-indigo-300"
              :disabled="loading || transitioning"
              @click="goToFollowUpForm('new')"
            >
              + 添加随访
            </button>
          </div>
          ...
          <div v-if="followUpCards.length" class="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-2">
            <article
              ...
              :key="item.id"
              class="relative rounded-2xl border border-slate-100 bg-slate-50 p-4 transition hover:border-indigo-200"
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                ...
                <button
                  ...
                  @click="goToFollowUpForm(item.id)"
                >
                  编辑
                </button>
              </div>
...
/**
 * 进入随访表单页（新增或编辑），并尽量保留当前详情页上下文。
 */
const goToFollowUpForm = (followUpId: number | string = 'new') => {
  if (!serviceCaseId.value) {
    return
  }

  router.push({
    path: `/mtm/service-cases/${serviceCaseId.value}/follow-ups/${followUpId}`,
    query: { ...route.query },
  })
}
