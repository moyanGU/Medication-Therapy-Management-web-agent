...
export function getPageTitle(routeName: string): string {
  ...
  const titleMap: Record<string, string> = {
    ...
    register: '注册',
    Dashboard: '仪表板',
    MtmServiceCases: '专业服务列表',
    MtmServiceCaseDetail: '专业服务详情',
    MtmInterviewForm: '药学问诊',
    MtmAssessmentForm: '用药评估',
    Medicines: '药品管理',
    MedicineDetail: '药品详情',
    Records: '用药记录',
    RecordStats: '用药统计',
...
export function getPagePurpose(routeName: string): string {
  ...
  const purposeMap: Record<string, string> = {
    ...
    MtmServiceCases: '查看全部专业服务记录与当前进度',
    MtmServiceCaseDetail: '查看专业用药指导服务的当前状态与摘要',
    MtmInterviewForm: '填写并保存专业服务中的药学问诊内容',
    MtmAssessmentForm: '填写并保存专业服务中的用药评估内容',
