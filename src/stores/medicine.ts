...
export const useMedicineStore = defineStore('medicine', () => {
  ...
  const fetchMedicines = async (params?: MedicineListParams) => {
    try {
      ...
      log('🔵 [Medicine Store] fetchMedicines 响应:', response)
      ...
      if (response.success) {
        ...
        if (isPaginated) {
          ...
          log(
            '🔵 [Medicine Store] 分页数据 results.length:',
            results.length
          )
          log(
            '🔵 [Medicine Store] 第一个药品的 image_path:',
            firstImagePath
          )
          ...
          const pageSize =
            actualData.page_size ?? actualData.pageSize ?? results.length
          const total = actualData.total ?? actualData.count ?? results.length
          const totalPages =
            actualData.total_pages ??
          ...
          log('🔵 [Medicine Store] 非分页数据 length:', list.length)
          // 非分页时，重置分页为单页
          pagination.value = {
            current: 1,
        ...
        logError('🔴 [Medicine Store] 服务器返回失败', {
          message: error.value,
        })
        // 向上抛出错误，供调用方处理
      ...
      if (err?.code === 499) {
        error.value = null
        log('🟡 [Medicine Store] 获取药品列表请求已取消')
        return
      }

      error.value = err.message || '获取药品列表失败'
      logError('获取药品列表失败', err)
      // 继续向上抛出，避免调用方误判成功
      throw err
    } finally {
      loading.value = false
  ...
  const createMedicine = async (
    ...
    log('🔵 [Medicine Store] createMedicine 开始执行', {
      data,
    ...
    try {
      ...
      log('🔵 [Medicine Store] 准备调用 medicineApi.createMedicine', {
        apiData: data,
      })

      const response = await medicineApi.createMedicine(data, config)

      log('🔵 [Medicine Store] medicineApi.createMedicine 响应', {
        response: response,
      ...
      logError('🔴 [Medicine Store] 创建药品异常', {
        error: err,
        message: err.message,
        stack: err.stack,
      ...
      loading.value = false
      log('🔵 [Medicine Store] createMedicine 执行完成', {
  ...
  const updateMedicine = async (
    ...
    try {
      ...
      const response = await medicineApi.updateMedicine(id, data, config)
