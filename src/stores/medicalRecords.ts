...
const handleCancelledMedicalRecordRequest = <T>(message: string, fallback: T) => {
  log(message)
  return fallback
}
...
export const useMedicalRecordStore = defineStore('medicalRecord', () => {
  ...
  const fetchRecords = async (params: any = {}) => {
    try {
      ...
      log('[medicalRecords] fetchRecords params:', queryParams)
      ...
      if (response.success) {
        ...
        const count =
          response.data?.pagination?.count ?? response.data?.count ?? 0
        records.value = list

        // 更新分页信息（优先采用服务端返回的分页信息）
        const serverPg = response.data?.pagination
        const pageSize = Number(
          serverPg?.page_size ?? pagination.value.pageSize
        )
        pagination.value.pageSize = pageSize
        pagination.value.total = Number(count)
        pagination.value.totalPages = Number(
          serverPg?.total_pages ?? Math.ceil(Number(count) / pageSize)
        )
        log('[medicalRecords] fetchRecords success:', {
          size: list.length,
      ...
      if (isRequestCancelledError(err)) {
        error.value = null
        return handleCancelledMedicalRecordRequest(
          '🟡 [medicalRecords] 获取病历列表请求已取消',
          undefined
        )
      }

      error.value = err.message || '获取病历列表失败'
      logError('获取病历列表失败', err)
      throw err
    } finally {
      loading.value = false
    }
  ...
  const fetchRecord = async (id: number) => {
    try {
      ...
      log('[medicalRecords] fetchRecord id=', id)
  ...
  const createRecord = async (
    ...
    try {
      ...
      log('[medicalRecords] createRecord payload:', recordData)
      const response = await api.post(BASE_PATH, recordData, config)
  ...
  const updateRecord = async (
    ...
    try {
      ...
      log(
        '[medicalRecords] updateRecord id=',
        id,
        'payload:',
        recordData
      )
      const response = await api.put(`${BASE_PATH}${id}/`, recordData, config)
  ...
  const deleteRecord = async (id: number) => {
    try {
      ...
      log('[medicalRecords] deleteRecord id=', id)
  ...
  const fetchCategories = async () => {
    try {
      log('[medicalRecords] fetchCategories')
      ...
      if (isRequestCancelledError(err)) {
        return handleCancelledMedicalRecordRequest(
          '🟡 [medicalRecords] 获取分类信息请求已取消',
          undefined
        )
      }

      logError('获取分类信息失败', err)
    }
  }
  ...
  const fetchStatistics = async (params: any = {}) => {
    try {
      log('[medicalRecords] fetchStatistics params:', params)
      ...
      if (statsParams.date_from)
        statsParams.date_from = toYMD(statsParams.date_from)
      if (statsParams.date_to) statsParams.date_to = toYMD(statsParams.date_to)

      log('[medicalRecords] fetchStatistics final params:', statsParams)
      const response = await api.get(`${BASE_PATH}statistics/`, {
        params: statsParams,
      })
      log('[medicalRecords] fetchStatistics response:', response)
      ...
      if (response.success) {
        ...
        log('[medicalRecords] normalized statistics:', statistics.value)
      }
    } catch (err: any) {
      if (isRequestCancelledError(err)) {
        return handleCancelledMedicalRecordRequest(
          '🟡 [medicalRecords] 获取统计信息请求已取消',
          undefined
        )
      }
  ...
  const fetchRecentRecords = async (days: number = 30) => {
    try {
      log('[medicalRecords] fetchRecentRecords days=', days)
      ...
      if (isRequestCancelledError(err)) {
        return handleCancelledMedicalRecordRequest(
          '🟡 [medicalRecords] 获取最近就诊记录请求已取消',
          undefined
        )
      }

      logError('获取最近就诊记录失败', err)
    }
  }
  ...
  const fetchFollowUpDue = async () => {
    try {
      log('[medicalRecords] fetchFollowUpDue')
      const response = await api.get(`${BASE_PATH}follow_up_due/`)
      ...
      if (isRequestCancelledError(err)) {
        return handleCancelledMedicalRecordRequest(
          '🟡 [medicalRecords] 获取复诊提醒请求已取消',
          undefined
        )
      }

      logError('获取复诊提醒失败', err)
    }
  }
