...
const handleCancelledRecordRequest = <T>(message: string, fallback: T) => {
  log(message)
  return fallback
}
...
export const useRecordStore = defineStore('record', () => {
  ...
  const normalizeRecordQuery = (params: MedicationRecordQuery = {}) => {
    ...
    if (normalized.page_size === undefined) {
      normalized.page_size = pagination.value.pageSize
    }
    if (normalized.page === undefined) {
      normalized.page = pagination.value.page
    }
  ...
  const fetchRecords = async (params?: MedicationRecordQuery) => {
    try {
      ...
      const requestParams = normalizeRecordQuery(queryParams.value)
      log('🔵 [Record Store] 获取记录列表参数:', requestParams)

      const response = await recordApi.getRecords(requestParams)

      if (!validateResponse(response, '获取记录列表')) {
        throw new Error('响应数据格式无效')
      }

      log('🔵 [Record Store] 记录列表响应:', response)

      if (response.success) {
        const data = response.data ?? {}
        log('🔵 [Record Store] 解析的data对象:', data)
        ...
        if (
          ...
          log(
            '🟢 [Record Store] 成功解析records数组，数量:',
            records.value.length
          )
        } else {
          warn('🟡 [Record Store] data.results不是数组，使用空数组')
          records.value = []
        }
        ...
        if (pg && typeof pg === 'object') {
          pagination.value = {
            page: Math.max(1, Number(requestParams.page ?? 1)),
            pageSize: Math.max(
              1,
              Number((requestParams as any).page_size ?? 20)
            ),
            total: Math.max(0, Number(pg.count ?? 0)),
            totalPages: Math.max(1, Number(pg.total_pages ?? 1)),
          }
          log('🟢 [Record Store] 分页信息更新:', pagination.value)
        } else {
          warn('🟡 [Record Store] 分页信息不可用')
        }

        log(
          '🟢 [Record Store] 记录列表获取成功，数量:',
          records.value.length
        )
      } else {
        const errorMsg = (response as any).message || '获取用药记录失败'
        logError('🔴 [Record Store] 获取记录列表失败', errorMsg)
        throw new Error(errorMsg)
      }
    } catch (err: any) {
      if (isRequestCancelledError(err)) {
        error.value = null
        return handleCancelledRecordRequest(
          '🟡 [Record Store] 获取记录列表请求已取消',
          undefined
        )
      }

      error.value = err.message || '获取用药记录失败'
      logError('🔴 [Record Store] 获取记录列表异常', err)
      records.value = []
    } finally {
      loading.value = false
    }
  ...
  const createRecord = async (data: MedicationRecordForm) => {
    try {
      ...
      log('🔵 [Record Store] 创建记录响应:', response)

      if (response.success) {
        log('🟢 [Record Store] 创建记录成功，准备更新records数组')
        log(
          '🟢 [Record Store] 当前records数组长度:',
          records.value.length
        )
        log('🟢 [Record Store] 新记录数据:', response.data)

        // 添加到记录列表开头
        records.value.unshift(response.data as MedicationRecord)
        log(
          '🟢 [Record Store] 更新后records数组长度:',
          records.value.length
        )

        // 更新统计信息
        await fetchStatistics()
        log('🟢 [Record Store] 统计信息已更新')
      ...
      logError('🔴 [Record Store] 创建记录异常', err)
      throw err
    } finally {
      loading.value = false
    }
  ...
  const updateRecord = async (
    id: number,
    data: Partial<MedicationRecordForm>
  ) => {
    try {
      loading.value = true
      error.value = null

      log('🔵 [Record Store] 更新记录，ID:', id, '数据:', data)
      ...
      log('🔵 [Record Store] 更新记录响应:', response)
      ...
      logError('🔴 [Record Store] 更新记录异常', err)
      throw err
    } finally {
      loading.value = false
    }
  ...
  const deleteRecord = async (id: number) => {
    try {
      ...
      log('🔵 [Record Store] 删除记录响应:', response)
      ...
      logError('🔴 [Record Store] 删除记录异常', err)
      throw err
    } finally {
      loading.value = false
    }
  ...
  const fetchStatistics = async (params?: {
    ...
    try {
      ...
      log('🔵 [Record Store] 获取统计信息参数:', params)
      ...
      log('🔵 [Record Store] 统计信息响应:', response)

      if (response.success) {
        statistics.value = response.data as MedicationRecordStats
        log('🟢 [Record Store] 统计信息获取成功:', statistics.value)
      } else {
      ...
      if (isRequestCancelledError(err)) {
        error.value = null
        return handleCancelledRecordRequest(
          '🟡 [Record Store] 获取统计信息请求已取消',
          undefined
        )
      }

      error.value = err.message || '获取统计信息失败'
      logError('🔴 [Record Store] 获取统计信息异常', err)
  ...
  const fetchTrends = async (params?: {
    ...
    try {
      ...
      log('🔵 [Record Store] 获取趋势数据参数:', params)
      ...
      log('🔵 [Record Store] 趋势数据响应:', response)

      if (response.success) {
        trends.value = Array.isArray(response.data)
          ? (response.data as MedicationTrend[])
          : []
        log(
          '🟢 [Record Store] 趋势数据获取成功，数量:',
          trends.value.length
        )
      } else {
        const errorMsg = (response as any).message || '获取趋势数据失败'
        logError('🔴 [Record Store] 获取趋势数据失败', errorMsg)
        throw new Error(errorMsg)
      }
    } catch (err: any) {
      if (isRequestCancelledError(err)) {
        error.value = null
        return handleCancelledRecordRequest(
          '🟡 [Record Store] 获取趋势数据请求已取消',
          undefined
        )
      }

      error.value = err.message || '获取趋势数据失败'
      logError('🔴 [Record Store] 获取趋势数据异常', err)
      trends.value = []
    } finally {
      loading.value = false
    }
  ...
  const fetchRecentRecords = async (limit: number = 10) => {
    try {
      ...
      log('🔵 [Record Store] 获取最近记录，限制数量:', limit)
      ...
      log('🔵 [Record Store] 最近记录响应:', response)

      if (response.success) {
        recentRecords.value = Array.isArray(response.data)
          ? (response.data as MedicationRecord[])
          : []
        log(
          '🟢 [Record Store] 最近记录获取成功，数量:',
          recentRecords.value.length
        )
      } else {
        const errorMsg = (response as any).message || '获取最近记录失败'
        logError('🔴 [Record Store] 获取最近记录失败', errorMsg)
        throw new Error(errorMsg)
      }
    } catch (err: any) {
      if (isRequestCancelledError(err)) {
        error.value = null
        return handleCancelledRecordRequest(
          '🟡 [Record Store] 获取最近记录请求已取消',
          undefined
        )
      }

      error.value = err.message || '获取最近记录失败'
      logError('🔴 [Record Store] 获取最近记录异常', err)
      recentRecords.value = []
    } finally {
      loading.value = false
    }
  ...
  const exportRecords = async (params?: {
    ...
    try {
      ...
      log('🔵 [Record Store] 导出记录参数:', params)

      const blob = await recordApi.exportRecords(params)

      log('🔵 [Record Store] 导出记录响应类型:', typeof blob)
      ...
      log('🟢 [Record Store] 记录导出成功')

      return true
    } catch (err: any) {
      error.value = err.message || '导出记录失败'
      logError('🔴 [Record Store] 导出记录异常', err)
      throw err
    } finally {
      loading.value = false
    }
  ...
  const validateResponse = (response: any, methodName: string): boolean => {
    if (!response || typeof response !== 'object') {
      logError(`🔴 [Record Store] ${methodName} 响应无效`, response)
      return false
    }
    ...
    if (
      ...
      logError(
        `🔴 [Record Store] ${methodName} 响应数据结构不符合ApiResponse`,
        response
      )
      return false
    }
