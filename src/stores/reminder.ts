...
const handleCancelledReminderRequest = <T>(message: string, fallback: T) => {
  log(message)
  return fallback
}

export const useReminderStore = defineStore('reminder', () => {
  const { success: showSuccess, error: showError, warning: showWarning } =
    useToast()
  ...
  const fetchReminders = async (params?: ReminderFilters) => {
    try {
      ...
      if (response.success) {
        ...
        log('[reminder] fetchReminders response.data:', pg)
        ...
        pagination.value = {
          ...
          totalPages:
            pg?.total_pages ??
            Math.ceil(
              (pg?.count ?? results.length) /
                (pg?.page_size ?? pagination.value.pageSize ?? 1)
            ),
        }
      } else {
      ...
      if (isRequestCancelledError(err)) {
        error.value = null
        return handleCancelledReminderRequest(
          '🟡 [reminder] 获取提醒列表请求已取消',
          undefined
        )
      }
  ...
  const createReminder = async (
    ...
    try {
      ...
      const response = await reminderService.createReminder(data, config)
  ...
  const updateReminder = async (
    ...
    try {
      ...
      const response = await reminderService.updateReminder(id, data, config)
  ...
  const toggleReminderActive = async (id: number) => {
    try {
      ...
      if (response.success) {
        ...
        showSuccess(
          response.message ??
            `成功更新 ${(response.data as any)?.updated_count ?? 0} 个提醒`
        )
        return response.data
      } else {
  ...
  const testNotification = async (
    ...
    try {
      ...
      if (response.success) {
        ...
        const message = response.message || '测试通知已触发'
        if (delivered) {
          showSuccess(message)
        } else {
          showWarning(message)
        }
        return { success: true, delivered, message }
      } else {
      ...
      showError(error.value)
      return { success: false, delivered: false, message: error.value }
    }
  }

  // 批量操作方法
  const batchToggleReminders = async (data: {
    reminder_ids: number[]
    is_active: boolean
  }) => {
    try {
      loading.value = true
      const response = await reminderService.batchToggleReminders(
        data.reminder_ids,
        data.is_active
      )
  ...
  const batchDeleteReminders = async (data: { reminder_ids: number[] }) => {
    try {
      loading.value = true
      const response = await reminderService.batchDeleteReminders(
        data.reminder_ids
      )
      ...
      if (response.success) {
        ...
        reminders.value = reminders.value.filter(
          r => !data.reminder_ids.includes(r.id)
        )
        showSuccess(
          response.message ??
            `成功删除 ${(response.data as any)?.deleted_count ?? 0} 个提醒`
        )
        return response.data
      } else {
  ...
  const fetchReminderHistory = async (params?: ReminderHistoryFilters) => {
    try {
      ...
      if (response.success) {
        ...
        log('[reminder] fetchReminderHistory payload:', payload)
        ...
        log('[reminder] fetchReminderHistory parsed:', {
          count,
          currentPage,
          pageSize,
          totalPages,
  ...
  const batchRespondToReminders = async (data: {
    history_ids: number[]
    ...
    try {
      loading.value = true
      const response = await reminderService.batchRespondToReminders(
        data.history_ids,
        data.response_type,
        data.notes
      )
      ...
      if (response.success) {
        ...
        showSuccess(
          response.message ??
            `成功响应 ${(response.data as any)?.updated_count ?? 0} 条提醒`
        )
        return response.data
      } else {
