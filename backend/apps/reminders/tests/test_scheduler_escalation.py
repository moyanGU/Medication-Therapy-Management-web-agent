...
class SchedulerEscalationTest(TestCase):
    ...
    def test_pending_push_failure_escalates_to_sms(self):
        ...
        count_sms = sum(
            ...
            if (x.notification_methods or []) and "sms" in x.notification_methods
        )
        self.assertEqual(count_sms, 1)

    def test_pending_escalation_deduplicates_existing_retry(self):
        h = ReminderHistory.objects.create(
            user=self.user,
            reminder=self.reminder,
            title="用药提醒",
        ...
        ReminderHistory.objects.create(
            ...
            scheduled_time=timezone.now() + timezone.timedelta(minutes=4),
        ...
        h.refresh_from_db()
        self.assertEqual(h.status, "failed")
        pending_sms = ReminderHistory.objects.filter(
            user=self.user,
            reminder=self.reminder,
            status="pending",
            reminder_type="repeat",
        ...
        self.assertEqual(pending_sms.count(), 1)

    def test_today_reminder_count_uses_sent_history_records(self):
        now = timezone.now()
        ReminderHistory.objects.create(
            user=self.user,
            reminder=self.reminder,
        ...
        ReminderHistory.objects.create(
            ...
            sent_at=now - timezone.timedelta(days=1),
            status="sent",
        )

        scheduler = ReminderScheduler()
        count = scheduler._get_today_reminder_count(self.reminder, now.date())
