...
class ReminderHistorySerializer(serializers.ModelSerializer):
    ...
    medicine_id = serializers.IntegerField(source='reminder.medicine.id', read_only=True)
    response_delay_display = serializers.CharField(source='get_response_delay_display', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            'id', 'reminder', 'reminder_title', 'medicine_name', 'medicine_id',
            'sent_at', 'scheduled_time', 'reminder_type', 'title', 'message',
            'notification_methods', 'status', 'responded_at', 'response_type',
            'response_delay_minutes', 'response_delay_display', 'notes',
            'device_info', 'is_responded', 'is_successful', 'created_at'
        ]
        read_only_fields = [
            'id', 'sent_at', 'response_delay_minutes', 'created_at'
        ]
...
class ReminderHistoryCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ReminderHistory
        fields = [
            'reminder', 'scheduled_time', 'reminder_type', 'title', 'message',
            'notification_methods', 'status', 'notes', 'device_info'
        ]
...
class ReminderStatsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ReminderStats
        fields = [
            'id', 'date', 'scheduled_count', 'sent_count', 'responded_count',
            'taken_count', 'skipped_count', 'delayed_count', 'ignored_count',
            'avg_response_time', 'response_rate', 'adherence_rate',
...
class ReminderEffectivenessSerializer(serializers.Serializer):
    ...
    trend_data = serializers.ListField(
        child=ReminderTrendSerializer()
    )
    
    # 药品分析
    medicine_analysis = serializers.ListField(
        child=MedicineReminderStatsSerializer()
    )
