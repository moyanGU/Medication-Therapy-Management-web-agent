...
from .history_serializers import (
    ...
    ReminderStatsDetailSerializer,
    ReminderAnalyticsSerializer,
    ReminderEffectivenessSerializer,
    ReminderComplianceSerializer
)
...
class ReminderHistoryViewSet(viewsets.ModelViewSet):
    ...
    def statistics(self, request):
        ...
        try:
            ...
            ).aggregate(avg=Avg('response_delay_minutes'))['avg'] or 0
            
            # 按响应类型统计
            response_type_stats = queryset.values('response_type').annotate(
                count=Count('id')
            ).order_by('-count')
            
            # 按药品统计
            medicine_stats = queryset.values(
                'reminder__medicine__id',
                'reminder__medicine__name'
            ).annotate(
    ...
    def metrics(self, request):
        try:
            ...
            push_sent = sum(1 for h in sent_qs if 'push' in (h.notification_methods or []))
            sms_sent = sum(1 for h in sent_qs if 'sms' in (h.notification_methods or []))
            email_sent = sum(1 for h in sent_qs if 'email' in (h.notification_methods or []))

            responded_count = sum(1 for h in sent_qs if h.response_type and h.response_type != 'no_response')
            response_rate = round((responded_count / len(sent_qs) * 100.0), 2) if sent_qs else 0.0

            channel_failed = {
                'push_failed': sum(1 for h in qs if h.status == 'failed' and 'push' in (h.notification_methods or [])),
                'sms_failed': sum(1 for h in qs if h.status == 'failed' and 'sms' in (h.notification_methods or [])),
                'email_failed': sum(1 for h in qs if h.status == 'failed' and 'email' in (h.notification_methods or [])),
            }
            ...
            escalation_success_rate = round((escalated_sent / escalated_total * 100.0), 2) if escalated_total else 0.0
    ...
    def trend(self, request):
        ...
        try:
            ...
            trend_data = queryset.order_by('date').values(
                'date', 'scheduled_count', 'sent_count', 'responded_count',
                'taken_count', 'response_rate', 'adherence_rate', 'avg_response_time'
            )
