...
class Reminder(models.Model):
    ...
    # 提醒时间
    reminder_time = models.TimeField(verbose_name="提醒时间", help_text="每日提醒的具体时间")
    ...
    dosage = models.PositiveIntegerField(
        validators=[MinValueValidator(1)], verbose_name="每次剂量", help_text="每次服用的药品数量"
    )
    ...
    is_active = models.BooleanField(
        default=True, verbose_name="是否激活", help_text="提醒是否处于激活状态"
    )

    # 提醒标题
    title = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="提醒标题", help_text="自定义提醒标题"
    )

    # 提醒内容
    message = models.TextField(
        blank=True, null=True, verbose_name="提醒内容", help_text="自定义提醒内容"
    )
    ...
    notification_types = models.JSONField(
        default=list, verbose_name="提醒方式", help_text="提醒的通知方式列表"
    )
    ...
    start_date = models.DateField(
        default=timezone.now, verbose_name="开始日期", help_text="提醒开始的日期"
    )

    # 结束日期
    end_date = models.DateField(
        blank=True, null=True, verbose_name="结束日期", help_text="提醒结束的日期，为空表示长期提醒"
    )

    # 星期设置（用于自定义频率）
    weekdays = models.JSONField(
        default=list, verbose_name="星期设置", help_text="自定义频率时的星期设置，1-7表示周一到周日"
    )
    ...
    special_instructions = models.TextField(
        blank=True, null=True, verbose_name="特殊说明", help_text="特殊的用药说明或注意事项"
    )

    # 最后提醒时间
    last_reminded_at = models.DateTimeField(
        blank=True, null=True, verbose_name="最后提醒时间", help_text="最后一次发送提醒的时间"
    )

    # 提醒次数统计
    reminder_count = models.PositiveIntegerField(
        default=0, verbose_name="提醒次数", help_text="累计提醒次数"
    )

    # 响应次数统计
    response_count = models.PositiveIntegerField(
        default=0, verbose_name="响应次数", help_text="用户响应提醒的次数"
    )

    # 创建时间
    created_at = models.DateTimeField(
        default=timezone.now, verbose_name="创建时间", help_text="提醒创建时间"
    )

    # 更新时间
    updated_at = models.DateTimeField(
        auto_now=True, verbose_name="更新时间", help_text="提醒最后更新时间"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["user_id", "is_active"], name="idx_reminders_user_active"
            ),
        ]
