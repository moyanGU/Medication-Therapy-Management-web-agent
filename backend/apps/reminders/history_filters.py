...
class ReminderHistoryFilter(django_filters.FilterSet):
    ...
    medicine = django_filters.ModelChoiceFilter(
        field_name="reminder__medicine", queryset=Medicine.objects.all(), label="药品"
    )

    medicine_name = django_filters.CharFilter(
        field_name="reminder__medicine__name", lookup_expr="icontains", label="药品名称"
    )

    # 新增：按提醒ID过滤（支持前端以 ?reminder=123 传参）
    reminder = django_filters.NumberFilter(field_name="reminder__id", label="提醒ID")

    # 提醒状态过滤
    status = django_filters.ChoiceFilter(
        choices=ReminderHistory.STATUS_CHOICES, label="发送状态"
    )

    # 响应类型过滤
    response_type = django_filters.ChoiceFilter(
        choices=ReminderHistory.RESPONSE_TYPE_CHOICES, label="响应类型"
    )

    # 提醒类型过滤
    reminder_type = django_filters.ChoiceFilter(
        choices=ReminderHistory.REMINDER_TYPE_CHOICES, label="提醒类型"
    )

    # 时间范围过滤
    sent_date = django_filters.DateFilter(field_name="sent_at__date", label="发送日期")

    sent_date_after = django_filters.DateFilter(
        field_name="sent_at__date", lookup_expr="gte", label="发送日期（起始）"
    )

    sent_date_before = django_filters.DateFilter(
        field_name="sent_at__date", lookup_expr="lte", label="发送日期（结束）"
    )

    sent_time_after = django_filters.TimeFilter(
        field_name="sent_at__time", lookup_expr="gte", label="发送时间（起始）"
    )

    sent_time_before = django_filters.TimeFilter(
        field_name="sent_at__time", lookup_expr="lte", label="发送时间（结束）"
    )

    # 响应时间过滤
    responded_date = django_filters.DateFilter(
        field_name="responded_at__date", label="响应日期"
    )

    responded_date_after = django_filters.DateFilter(
        field_name="responded_at__date", lookup_expr="gte", label="响应日期（起始）"
    )

    responded_date_before = django_filters.DateFilter(
        field_name="responded_at__date", lookup_expr="lte", label="响应日期（结束）"
    )

    # 响应延迟过滤
    response_delay_min = django_filters.NumberFilter(
        field_name="response_delay_minutes", lookup_expr="gte", label="响应延迟（最小分钟）"
    )

    response_delay_max = django_filters.NumberFilter(
        field_name="response_delay_minutes", lookup_expr="lte", label="响应延迟（最大分钟）"
    )

    # 布尔过滤
    is_responded = django_filters.BooleanFilter(
        method="filter_is_responded", label="是否已响应"
    )

    is_successful = django_filters.BooleanFilter(
        method="filter_is_successful", label="是否成功"
    )

    # 通知方式过滤
    has_push_notification = django_filters.BooleanFilter(
        method="filter_has_push_notification", label="包含推送通知"
    )

    has_email_notification = django_filters.BooleanFilter(
        method="filter_has_email_notification", label="包含邮件通知"
    )

    has_sms_notification = django_filters.BooleanFilter(
        method="filter_has_sms_notification", label="包含短信通知"
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            "sent_time_before",
            "responded_date",
            "responded_date_after",
            "responded_date_before",
            "response_delay_min",
            "response_delay_max",
            "is_responded",
            "is_successful",
    ...
    def filter_is_successful(self, queryset, name, value):
        ...
        """
        if value is True:
            return queryset.filter(status="sent").exclude(response_type="no_response")
        elif value is False:
...
class ReminderStatsFilter(django_filters.FilterSet):
    ...
    date = django_filters.DateFilter(field_name="date", label="统计日期")

    date_after = django_filters.DateFilter(
        field_name="date", lookup_expr="gte", label="统计日期（起始）"
    )

    date_before = django_filters.DateFilter(
        field_name="date", lookup_expr="lte", label="统计日期（结束）"
    )

    response_rate_min = django_filters.NumberFilter(
        field_name="response_rate", lookup_expr="gte", label="响应率（最小%）"
    )

    response_rate_max = django_filters.NumberFilter(
        field_name="response_rate", lookup_expr="lte", label="响应率（最大%）"
    )

    adherence_rate_min = django_filters.NumberFilter(
        field_name="adherence_rate", lookup_expr="gte", label="服药率（最小%）"
    )

    adherence_rate_max = django_filters.NumberFilter(
        field_name="adherence_rate", lookup_expr="lte", label="服药率（最大%）"
    )

    sent_count_min = django_filters.NumberFilter(
        field_name="sent_count", lookup_expr="gte", label="发送数量（最小）"
    )

    sent_count_max = django_filters.NumberFilter(
        field_name="sent_count", lookup_expr="lte", label="发送数量（最大）"
    )

    avg_response_time_min = django_filters.NumberFilter(
        field_name="avg_response_time", lookup_expr="gte", label="平均响应时间（最小分钟）"
    )

    avg_response_time_max = django_filters.NumberFilter(
        field_name="avg_response_time", lookup_expr="lte", label="平均响应时间（最大分钟）"
    )
    ...
    def filter_time_range(self, queryset, name, value):
        ...
        if value == "this_week":
            ...
            end_of_last_week = start_of_this_week - timedelta(days=1)
            return queryset.filter(
                date__gte=start_of_last_week, date__lte=end_of_last_week
            )
        elif value == "this_month":
