...
class ReminderFilter(django_filters.FilterSet):
    ...
    frequency = django_filters.ChoiceFilter(
        field_name="frequency", choices=Reminder.FREQUENCY_CHOICES
    )

    # 餐前餐后过滤
    meal_timing = django_filters.ChoiceFilter(
        field_name="meal_timing", choices=Reminder.MEAL_TIMING_CHOICES
    )
    ...
    reminder_time_from = django_filters.TimeFilter(
        field_name="reminder_time", lookup_expr="gte"
    )
    reminder_time_to = django_filters.TimeFilter(
        field_name="reminder_time", lookup_expr="lte"
    )
    ...
    # 今天是否提醒过滤
    remind_today = django_filters.BooleanFilter(method="filter_remind_today")

    # 响应率范围过滤
    response_rate_min = django_filters.NumberFilter(method="filter_response_rate_min")
    response_rate_max = django_filters.NumberFilter(method="filter_response_rate_max")

    # 提醒次数范围过滤
    reminder_count_min = django_filters.NumberFilter(
        field_name="reminder_count", lookup_expr="gte"
    )
    reminder_count_max = django_filters.NumberFilter(
        field_name="reminder_count", lookup_expr="lte"
    )
