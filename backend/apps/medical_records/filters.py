...
class MedicalRecordFilter(django_filters.FilterSet):
    ...
    visit_type = django_filters.ChoiceFilter(
        choices=MedicalRecord.VISIT_TYPE_CHOICES,
        label='就诊类型'
    )
    
    # 就医状态过滤
    status = django_filters.ChoiceFilter(
        choices=MedicalRecord.STATUS_CHOICES,
        label='就医状态'
    )
    
    # 紧急程度过滤
    urgency = django_filters.ChoiceFilter(
        choices=MedicalRecord.URGENCY_CHOICES,
        label='紧急程度'
    )
    ...
    visit_year = django_filters.NumberFilter(
        field_name='visit_date__year',
        label='就诊年份'
    )
    
    # 按月份过滤
    visit_month = django_filters.NumberFilter(
        field_name='visit_date__month',
        label='就诊月份'
    )
    ...
    class Meta:
        model = MedicalRecord
        fields = [
            'visit_date_from', 'visit_date_to', 'hospital', 'department',
            'doctor', 'diagnosis', 'chief_complaint', 'visit_type', 'status',
            'urgency', 'total_cost_min', 'total_cost_max', 'satisfaction_score',
            'satisfaction_score_min', 'has_follow_up', 'follow_up_due',
            'recent_days', 'search', 'visit_year', 'visit_month',
            'diagnosis_code', 'doctor_title'
        ]
