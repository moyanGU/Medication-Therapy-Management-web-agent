...
class MedicalRecord(models.Model):
    ...
    # 就诊日期
    visit_date = models.DateField(verbose_name="就诊日期", help_text="实际就诊的日期")

    # 就诊时间
    visit_time = models.TimeField(
        blank=True, null=True, verbose_name="就诊时间", help_text="具体的就诊时间"
    )

    # 医院名称
    hospital = models.CharField(
        max_length=100, verbose_name="医院名称", help_text="就诊的医院名称"
    )

    # 医院地址
    hospital_address = models.CharField(
        max_length=200, blank=True, null=True, verbose_name="医院地址", help_text="医院的详细地址"
    )

    # 科室
    department = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="科室", help_text="就诊的科室"
    )

    # 医生姓名
    doctor = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="医生姓名", help_text="接诊医生的姓名"
    )
    ...
    chief_complaint = models.TextField(
        blank=True, null=True, verbose_name="主诉", help_text="患者的主要症状和不适"
    )

    # 现病史
    present_illness = models.TextField(
        blank=True, null=True, verbose_name="现病史", help_text="当前疾病的发展过程"
    )

    # 诊断结果
    diagnosis = EncryptedTextField(
        blank=True, null=True, verbose_name="诊断结果", help_text="医生的诊断结果"
    )
    ...
    treatment = EncryptedTextField(
        blank=True, null=True, verbose_name="治疗方案", help_text="医生制定的治疗方案"
    )

    # 处方药品
    prescribed_medicines = models.JSONField(
        default=list, verbose_name="处方药品", help_text="医生开具的药品列表"
    )

    # 检查项目
    examinations = models.JSONField(
        default=list, verbose_name="检查项目", help_text="进行的检查项目列表"
    )

    # 检查结果
    examination_results = models.TextField(
        blank=True, null=True, verbose_name="检查结果", help_text="各项检查的结果"
    )

    # 化验结果
    lab_results = models.JSONField(
        default=dict, verbose_name="化验结果", help_text="化验检查的具体数值结果"
    )

    # 医嘱
    medical_orders = EncryptedTextField(
        blank=True, null=True, verbose_name="医嘱", help_text="医生的医嘱和建议"
    )

    # 注意事项
    notes = models.TextField(
        blank=True, null=True, verbose_name="注意事项", help_text="就医过程中的备注和注意事项"
    )
    ...
    follow_up_date = models.DateField(
        blank=True, null=True, verbose_name="复诊时间", help_text="建议的复诊时间"
    )

    # 复诊说明
    follow_up_notes = models.TextField(
        blank=True, null=True, verbose_name="复诊说明", help_text="复诊的相关说明"
    )
    ...
    attachments = models.JSONField(
        default=list, verbose_name="相关文件", help_text="相关的医疗文件、报告等"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["user_id", "visit_date"], name="idx_medical_records_user_date"
            ),
        ]
    ...
    def get_examination_names(self):
        ...
        if self.examinations:
            return [
                exam.get("name", "") for exam in self.examinations if exam.get("name")
            ]
        return []
    ...
    def get_recent_visits(cls, user, days=30):
        ...
        return cls.objects.filter(user=user, visit_date__gte=cutoff_date).order_by(
            "-visit_date"
        )
    ...
    def get_visits_by_hospital(cls, user, hospital_name):
        ...
        return cls.objects.filter(
            user=user, hospital__icontains=hospital_name
        ).order_by("-visit_date")
    ...
    def get_visits_by_department(cls, user, department_name):
        ...
        """
        return cls.objects.filter(
            user=user, department__icontains=department_name
        ).order_by("-visit_date")
