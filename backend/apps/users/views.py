...
from .serializers import UserProfileSerializer, PushSubscriptionSerializer
...
def push_subscriptions(request):
    ...
    if request.method == 'POST':
        ...
        payload = {
            ...
            'user_agent': request.data.get('ua') or request.META.get('HTTP_USER_AGENT', ''),
        ...
        if sub:
            ...
            return Response({'success': True, 'data': {'id': sub.id, 'endpoint': sub.endpoint}, 'message': '更新成功'})
        ...
        if serializer.is_valid():
            ...
            return Response({'success': True, 'data': {'id': sub.id, 'endpoint': sub.endpoint}, 'message': '保存成功'}, status=status.HTTP_201_CREATED)
        else:
    ...
    if not endpoint:
        ...
        return Response({'success': False, 'data': None, 'message': '缺少 endpoint'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if deleted:
        ...
        return Response({'success': False, 'data': None, 'message': '订阅不存在'}, status=status.HTTP_404_NOT_FOUND)